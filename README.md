...
- **stretching-owner self-constraint:** the genuine record owner `P=omega.S.omega` obeys its own short NSE law `D_tP=omega.S^2.omega-omega.Hess(p).omega+V_P`; pressure curvature is generated nonlocally by incompressibility and can either reinforce or overpower self-strain on exact solutions, so owner persistence is a second endogenous PDE gate rather than an autonomous recurrence rule.
- **endogenous level-set moving readout:** regular enstrophy superlevel boundaries obey `(V-u).n=(R-dot lambda)/|grad e|`; coarea integrates these local owner fluxes exactly to the global enstrophy law and hence to the certified spectral split/merge ledger, while current Kelvin ancestry readout uses the same signed boundary flux as population revaluation rather than a new covariance source.
- **enstrophy value-space current:** pushing physical volume through `e` gives the exact distributional law `partial_t mu+partial_a j=0`; regular level sets, superlevel motion and global moments are readings of this one owner current.  A unique nondegenerate record edge is its characteristic endpoint, while a tied edge keeps an additional extremal lineage-selection law rather than collapsing to the bulk conditional average.
- **support-edge owner-renewal grammar:** the active-set `theta->1` compatibility numerator selects the advancing edge intrinsically; on a unique nondegenerate maximizing lineage the similarity-invariant stretching efficiency obeys `d_tau sigma=-sigma^2+N_renew`.  Eventual `N_renew<=0` forces only quadratic growth in intrinsic time and excludes finite-physical-time record blow-up on that branch; any hypothetical finite-time unbounded record must instead renew `N_renew>0` arbitrarily late or leave the lineage theorem domain.
- **analytic-Morse selector finiteness:** on every compact positive-time classical interval where the complete enstrophy critical set remains Morse, positive-time NSE analyticity makes that set a finite analytic lineage covering; branch-value crossings are finite modulo persistent ties, so interior scalar winner-Zeno forces Hessian degeneracy rather than an independent selector mechanism.
- **transverse-support separation:** current read-only Kelvin `1095c13` shows scalar chamber thickness, packet volume, current residual, q.v. and normal ancestry flux can collapse while `B_parallel=P L L^T P` stays `O(1)`; scalar/normal localization therefore does not replace tangential physical support or frame ancestry.
- **record-contact core funding gate:** vorticity-adapted strain plus pressure Poisson split local renewal into turning, transverse strain shape, trace-free pressure anisotropy and viscosity; positive running-record renewal must beat the exact tax `Sigma^2/2+1/3`, with geometry harvesting and normalization relief retained as separate faces.
...
Latest read-only upstream convergence: Kelvin `5067f87` independently derives the similarity-invariant max-normalized enstrophy filtration `e/M` and the same relative-boundary numerator `R-theta Mdot`; Wang `7b91828` makes raw material address/relink labels fail closed unless an independently certified physical supplier exists.  Both reinforce the repo-3 owner-first ordering without identifying their distinct currencies.
