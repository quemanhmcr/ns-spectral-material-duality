...
## Theorem BQ — Every radial record gate has a quantitative resolved/local/UV-skip owner

At a BN gate `F(R)>=nu G(R)`, actual upward mass satisfies `Phi_up>=nu G`. Split it by donor radius below/above `R/4`. If the far-low branch carries at least half, it is literal hard low--high incidence. With `V_R=P_(|k|<R/4)u`, the exact mixed operator on the divergence-free subspace splits as `L_(V_R)=mathsf K_R+mathsf S_R`, `mathsf K_R^*=-mathsf K_R`, `mathsf S_R^*=mathsf S_R`, with `mathsf S_R=P(S_R .)`. Splitting each far edge before modal summation, skew same-event redistribution or symmetric strain work carries at least `nu G/4`. Otherwise the near-boundary branch carries at least half. Splitting that branch by recipient radius `4R` yields either local outward work at least `nu G/4` with `|k_r|/|k_d|<=16`, or UV-skip work at least `nu G/4`. Every skip atom has a closed-triad companion satisfying `|k_c|>3R`. Thus only the skip branch retains a genuine high-companion question; exact ties remain joint.
